        ...
        <div class="col-md-12">
            <div class="panel panel-inverse" data-sortable-id="form-validation-1">
                ...
                <div class="panel-body panel-form">
                    <form class="form-horizontal form-bordered" action="{{ url('/admin/categories') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group"><!-- 父级 -->
                            ...
                            <div class="col-md-6 col-sm-6">
                                <select name="parent_id" value="{{ old('') }}" placeholder="父级" class="form-control" id="parent_id">
                                    <option value="0">顶级</option>
                                    @if ( $categories )
                                        @foreach ( $categories as $id => $title )
                                            <option value="{{ $id }}" @if ( old('parent_id')==$id ) @endif>{{ $title }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
