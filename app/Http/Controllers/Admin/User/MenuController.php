...
class MenuController extends Controller {
    ...
    public function index(Request $request) {
        if ( $request->ajax() ) {
            ...
            $model = Menu::where('created_by', auth('admin')->user()->id); // 筛选用户
            ...
            $parent = null;
            if ( $parent_id ) $parent = Menu::find($parent_id);

            return view('admin.user.menu.index', compact('types', 'search', 'parent'));
        }
    }
    ...
    public function store(MenuRequest $request) {
        $result = Menu::create(array_merge($request->all(), [
        ...
        if ( $result ) {
            ...
            return redirect('/admin/menus'); // 列表
        } else {
            flash('操作失败', 'error');
    ...
    public function show(int $id) {
        return 'user menu show';
    }
    public function edit(int $id) {
        $item = Menu::find($id);

        return view('admin.user.menu.edit', compact('menus', 'item'));
    }
